...
Packet.prototype._parseData = function(parser, payload, ds) {
  ...
  if (parser && (data.length > 0)) {
    ...
    var dsIndex = 0, dsFlag = false;

    for (var i = 0; i < fields.length; i++) {
      field = fields[i];
      // Since we are using 32bit integers when we use the MSB position
      // we get a negative value, we need to change to absolute in order
      // to be accurate.
