...
Packet.prototype.parseAsyncData = function(payload) {
  var data = payload.data,
      parsedData = {},
      parser = ASYNC_PARSER[payload.idCode];

  parsedData.idCode = parser.idCode;
  parsedData.event = parser.event;
  parsedData.DID = parser.DID;
  parsedData.CID = parser.CID;
  parsedData.packet = payload;
  ...
  if (data) {
    for (var i = 0; i < fields.length; i++) {
      ...
      if (field.type === "number") {
        parsedData[field.name] = utils.bufferToInt(
          data.slice(field.from, field.to)
        );
      } else if (field.type === "string") {
        parsedData[field.name] = data.toString("utf8", field.from, field.to);
      }
    }
  }
...
Packet.prototype.parseResponseData = function() {
};
