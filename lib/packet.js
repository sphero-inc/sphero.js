...
Packet.prototype._parseData = function(parser, payload, ds) {
  ...
  if (parser && (data.length > 0)) {
    ...
    // if data streaming, both bitmasks are required
    ...
    while (i < fields.length) {
      ...
      pData[field.name] = this._parseField(field, data, pData, dsFlag);

      if (pData[field.name] === undefined) {
        delete pData[field.name];
      }
...
Packet.prototype._checkDS = function(ds, field) {
  if (!ds) {
    return -1;
  }
...
Packet.prototype._parseField = function(field, data, pData, dsFlag) {
  if (dsFlag === 0) {
    return undefined;
  }
  ...
  parsedField = utils.bufferToInt(data);
  ...
  switch (field.type) {
    case "number":
      ...
      break;
    case "string":
      parsedField = data.toString(field.format).replace(/\0/g, "0");
      break;
    case "raw":
      parsedField = new Buffer(data);
      break;
    case "predefined":
      if (field.mask != null) {
        parsedField &= field.mask;
      }
      parsedField = field.values[parsedField];
      break;
    case "bitmask":
      val = parsedField;
      ...
      if (pData[field.name]) {
        parsedField = pData[field.name];
        parsedField.values.push(val);
      } else {
        parsedField = {
          sensor: field.sensor,
          range: field.range,
