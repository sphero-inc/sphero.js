...
Packet.prototype.command = function(cid, data, opts) {
  data = data || [];
  opts = opts || {};

  var sop1 = opts.sop1 || 0xFF,
      sop2 = opts.sop2 || 0xFC,
      seq = opts.seq || 0x00,
      // Add 1 to dlen, since it also counts the checksum byte
      dlen = data.length + 1,
      did = opts.did || this.did,
      checksum = 0x00;
...
Packet.prototype.parse = function(buffer) {
  ...
  if (buffer[0] === 0xFF) {
    // If valid check SOP2 to be 0xFF or 0xF
    // SOP2 === 0xFF when acknoledgement
