...
module.exports = function custom(device) {
  ...
  /**
   * Starts streaming of quaternions
   *
   * It uses sphero's data streaming command. User needs to listen
   * for the `dataStreaming` or `quaternions` event to get the data.
   *
   * @param {Number} [sps=5] samples per second
   * @param {Boolean} [remove=false] forces streamin of quaternions to stop
   * @example
   * orb.streamQuaternions();
   *
   * orb.on("quaternions", function(data) {
   *   console.log("data:");
   *   console.log("  quaternionQ0:", data.quaternionQ0);
   *   console.log("  quaternionQ1:", data.quaternionQ1);
   *   console.log("  quaternionQ2:", data.quaternionQ2);
   *   console.log("  quaternionQ3:", data.quaternionQ3);
   * });
   * @return {object} promise for command
   */
  device.streamQuaternions = function(sps, remove) {
