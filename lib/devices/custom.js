...
module.exports = function custom(device) {
   ...
   *
   * @param {Object} args { eventName, mask1, mask2, fields, sps } (hex)
   * @param {Boolean} remove=false forces velocity streaming to stop
   * @return {void}
   */
  device._streamData = function(args) {
    // options for streaming data
    var opts = {
      n: Math.round(400 / (args.sps || 2)),
      m: 1,
      mask1: this._mergeMasks("mask1", args.mask1, args.remove),
      pcnt: 0,
      mask2: this._mergeMasks("mask2", args.mask2, args.remove)
    };

    var callback = function(data) {
      console.log("CHECKPOINT ==>", args.event);
      device.emit(args.event, data);
    };
