...
module.exports = function sphero(device) {
  ...
  device.setHeading = function(heading, callback) {
    ...
    command(commands.setCal, heading, callback);
  };

  device.setStabilization = function(flag, callback) {
    flag &= 0x01;
    command(commands.setCal, [flag], callback);
  };
  ...
  device.reEnableDemo = function(callback) {
    command(commands.reEnableDemo, null, callback);
  };
  ...
  device.configureCollisionDetection = function(opts, cb) {
    var data = [
      opts.meth,
      opts.xt,
      opts.xs,
  ...
  device.setAccelerometerRange = function(ldx, callback) {
    ldx &= ldx;
  ...
  device.setMotionTimeout = function(time, callback) {
    ...
    command(commands.setMotionTo, time, callback);
  };
