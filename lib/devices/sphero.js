...
module.exports = function Core() {
  ...
  sphero = {
    ...
    _xpAndLevelUp: function(cmd, pw, gen, cb) {
      pw = utils.intToHexArray(pw, 4);
      gen &= 0xFF;

      this.spheroCommand(cmd, [].concat(pw, gen), cb);
    },

    addXp: function(pw, qty, callback) {
      this.xpOrLevelUp(commands.ssbAddXp, pw, qty, callback);
    },

    levelUpAttr: function(pw, id, callback) {
      this.xpOrLevelUp(commands.ssbLevelUpAttr, pw, id, callback);
    },
