...
module.exports = function sphero(device) {
   ...
   *
   * After both limits are satisified, option bits control sleep, final
  ...
  /**
   * The Set Back LED command allows brigthness adjustment of Sphero's tail
   * light.
   *
  ...
  /**
   * The Set Motion Timeout command gives Sphero an ultimate timout for the last
   * motion command to keep Sphero from rolling away in the case of a crashed
   * (or paused) application.
   *
  ...
  /**
   * The Get Permanent Option Flags comamnd returns Sphero's permanent option
   * flags, as a bitfield:
   *
   * Bit # | Description
   * ----- | -----------
  ...
  /**
   * The Set Temporrary Option Flags command assigns Sphero's temporary option
  ...
  /**
   * The Get Temporary Option Flags comamnd returns Sphero's temporary option
   * flags, as a bitfield:
   *
   * Bit # | Description
   * ----- | -----------
   ...
   * blocks.
   *
   * The response is a simple one; an error code of 08h is returned when the
   ...
   * 04h due to its length (if there is no error).
   *
   * ID = 00h requests the factory configuration block
   * ID = 01h requests the user configuration block, which is updated with
   * current values first
   *
   * @param {Number} id which configuration block to fetch
   * @param {Function} callback to be triggered after writing
   * @return {void}
   */
